...
def rank_head(attn_weights: torch.Tensor):
    """Rank redundancy of attention heads by computing their traces
    batch * heads * time * time
    """
    traces: List[torch.Tensor] = []

    for batch in attn_weights:
        traces.append(torch.Tensor([head.trace() for head in batch]))

    return torch.stack(traces).mean(dim=0).argsort(descending=True)


def kr_vad(label: torch.Tensor, attn_head_weights: torch.Tensor):
    """
    pred: batch * time * speakers
    label: batch * time * speakers
    """
    n_batch, n_frame, n_speaker = label.shape

    head_rank = rank_head(attn_head_weights)
    label[label < 0] = 0
    label[label > 0] = 1
    ...
    for spk in range(n_speaker):
        mask = label.permute(2, 0, 1)[spk]
        mask = mask.reshape(n_batch, -1, 1)
        mask = torch.bmm(mask, mask.permute(0, 2, 1))

        attn = attn_head_weights.permute(1, 0, 2, 3)[head_rank[spk]]
        losses.append(BCE(attn, mask))

    # loss = torch.stack(losses).mean()
    loss = torch.stack(losses).sum()
    # logger.info(loss)
    return loss, head_rank[:n_speaker]
